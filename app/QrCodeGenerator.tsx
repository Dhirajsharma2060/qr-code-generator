...
function QrCodeGenerator() {
  ...
  const [error, setError] = React.useState<string | null>(null); // Line 19

  const handleDownload = (type: "png" | "svg") => {
    const qrCodeElem = document.getElementById("qr-code");
  
    if (!url) {
      setError("Please enter a URL or generate an email QR code.");
      return; // Prevent download if URL is empty
    }
  ...
  const handleEmailInput = () => {
    if (!email) { // Line 43
      setError("Email is required."); // Line 44
      return; // Line 45
    }
    if (!subject) { // Line 46
      setError("Subject is required."); // Line 47
      return; // Line 48
    }
    if (!message) { // Line 49
      setError("Message is required."); // Line 50
      return; // Line 51
    }
    ...
    setUrl(mailToLink);
    setError(null); // Clear any previous error // Line 55
  };
              ...
              >
                ...
                <TabsContent value="link">
                  <div className="space-y-6">
                    ...
                    {error && qrType === "link" && ( // Line 73
                      <div className="text-red-500">{error}</div> // Line 74
                    )}
                  </div>
                </TabsContent>
                ...
                <TabsContent value="email">
                  <div className="space-y-4">
                    ...
                    {error && qrType === "email" && ( // Line 91
                      <div className="text-red-500">{error}</div> // Line 92
                    )}
                    <div className="space-y-2">
                      <Label
                        htmlFor="subject"
                  ...
                  <div className="space-y-2 flex-1">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      >
                        <input
                          type="color"
                          value={bgColor} // Changed to bgColor
                          onChange={(e) => setBgColor(e.target.value)} // Changed to bgColor
                          className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
                        />
                      </div>
                      <Input
